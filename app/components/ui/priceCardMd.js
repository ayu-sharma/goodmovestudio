...
const PriceCardMd = ({
  ...
  const initialSelectedPlanId = Array.isArray(plans) && plans.length > 0 ? plans[0]?.id : null;
  const [selectedPlanId, setSelectedPlanId] = useState(initialSelectedPlanId);
  const { formatDate } = useData();
  const dateInfo =
  formatDate && date ? formatDate(date) : null;
  return (
    <div className="bg-white border border-gray-300 md:rounded-2xl rounded-t-3xl py-4 px-5 shadow-lg">
      ...
      <div className="space-y-4 mb-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <span className="text-gray-700 text-sm md:text-base">{dateInfo?.formattedDate}</span>
        </div>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-baseline">
                      ...
                      {/* <span className="text-gray-500 text-sm ml-1">
                        {plan.name === "Monthly Basic" ? "/month" : "/quarter"}
                      </span> */}
                    </div>
                    {plan.discount && (
