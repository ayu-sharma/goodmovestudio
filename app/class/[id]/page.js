...
const Eventpage = () => {
  ...
  const { getClassById, loading } = useData();
  const eventData = getClassById(parseInt(params.id));

  // Format days with comma separation for multiple days
...
console.log("eventData", eventData)
  // Format time object to display start and end times
  const formatTime = (timeObj) => {
    if (!timeObj) return 'TBD';
